...
def plot_data_distribution(domain, x_interior, y_interior, lb, ub):
...
def train_pinn(
    ...
    def closure():
        ...
        # 입구 경계 조건
        u_pred, v_pred, p_pred = model.net_uvp(x_batch, y_batch, t_batch)
        u_inlet, v_inlet, p_inlet = inlet_bc.apply(x_batch, y_batch, u_pred.clone(), v_pred.clone(), p_pred.clone())
        loss_inlet_bc = torch.mean((u_inlet - u_pred)**2) + torch.mean((v_inlet - v_pred)**2)

        # 상단 벽 경계 조건
        u_top, v_top, p_top = top_bc.apply(x_batch, y_batch, u_pred, v_pred, p_pred)
        loss_top_bc = torch.mean((u_top - u_pred)**2) + torch.mean((v_top - v_pred)**2)

        # 하단 벽 경계 조건
        u_bottom, v_bottom, p_bottom = bottom_bc.apply(x_batch, y_batch, u_pred, v_pred, p_pred)
        loss_bottom_bc = torch.mean((u_bottom - u_pred)**2) + torch.mean((v_bottom - v_pred)**2)

        # 출구 압력 경계 조건
        _, _, p_outlet = outlet_p_bc.apply(x_batch, y_batch, u_pred, v_pred, p_pred)
        loss_outlet_p_bc = torch.mean((p_outlet - p_pred)**2)

        # 출구 속도 기울기 경계 조건
        u_b_outlet_dy, v_b_outlet_dy, _ = outlet_du_dy_bc.apply(x_batch, y_batch, u_pred, v_pred, p_pred)
        loss_outlet_du_dy_bc = (torch.mean((u_b_outlet_dy - u_pred)**2) +
                                torch.mean((v_b_outlet_dy - v_pred)**2))

        # 실린더 노-슬립 경계 조건 손실
        u_wall, v_wall, p_wall = wall_bc.apply(x_batch, y_batch, u_pred.clone(), v_pred.clone(), p_pred.clone())
        ...
        u_initial, v_initial, p_initial = model.net_uvp(x_batch, y_batch, t_initial)
...
# Function to create and save the unsteady visualization
def create_unsteady_visualization_data(domain, model_mlp, num_x, num_y, lb, ub, time_values):
    ...
    for t in time_values:
        x_flat = torch.tensor(X.flatten(), dtype=torch.float32, device=device).unsqueeze(1)
        y_flat = torch.tensor(Y.flatten(), dtype=torch.float32, device=device).unsqueeze(1)
        t_flat = torch.full_like(x_flat, t)  # Create a tensor of time values

        # Predict u, v, p using your model
        u, v, p = model_mlp.net_uvp(x_flat, y_flat, t_flat)

        # Reshape predictions to match the grid and append to data lists
    ...
    # Initial contours (for the first frame)
    cont1 = axs[0].contourf(X, Y, u_data[0], cmap='jet')
    fig.colorbar(cont1, ax=axs[0], orientation='horizontal')
    cont2 = axs[1].contourf(X, Y, v_data[0], cmap='jet')
    fig.colorbar(cont2, ax=axs[1], orientation='horizontal')
    cont3 = axs[2].contourf(X, Y, p_data[0], cmap='jet')
    fig.colorbar(cont3, ax=axs[2], orientation='horizontal')
    ...
    # Add circle representing the cylinder to each subplot
    cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black', zorder=10)
    axs[0].add_patch(cylinder_plot)
    cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black', zorder=10)
    axs[1].add_patch(cylinder_plot)
    cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black', zorder=10)
    axs[2].add_patch(cylinder_plot)
    ...
    # Add circle representing the cylinder to each subplot
        cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black')
        axs[0].add_patch(cylinder_plot)
        cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black')
        axs[1].add_patch(cylinder_plot)
        cylinder_plot = plt.Circle((cylinder_center[0], cylinder_center[1]), cylinder_radius, color='black')
        axs[2].add_patch(cylinder_plot)
        cont1 = axs[0].contourf(X, Y, u_data[i], cmap='jet')
        cont2 = axs[1].contourf(X, Y, v_data[i], cmap='jet')
        cont3 = axs[2].contourf(X, Y, p_data[i], cmap='jet')
        ...
        return cont1, cont2, cont3  # Return updated contours

    # Create the animation
    ani = animation.FuncAnimation(fig, animate, frames=len(time_values), interval=200, blit=False)

    ani.save('unsteady_flow.gif', writer='pillow', fps=5)  # Use pillow writer and set FPS
    #plt.show()

    return X, Y, u_data, v_data, p_data  # return X, Y and u, v, p data
...
if __name__ == "__main__":
    ...
    ub = np.array([2, 0.7])    # 도메인 상한 (x_max, y_max)

    cylinder_center = np.array([0.5, 0.35])  # 원의 중심
    ...
    # 데이터 분포 시각화
    plot_data_distribution(domain, x_interior, y_interior, lb, ub)

    # ----------------------------
    # 4. PINN 모델 초기화
    # ----------------------------

    layers_mlp = [3, 50, 50, 50, 50, 50, 50 ,50, 3]
    model_mlp = PINN_MLP_unsteady(domain, layers_mlp, lb, ub, mu=mu, rho=rho).to(device)

    # ----------------------------
    # 5. 손실 가중치 정의
    # ----------------------------
    bc_weights = {'inlet': 9.0, 'top': 1.0, 'bottom': 1.0, 'outlet_p': 1.0, 'outlet_du_dy': 1.0, 'wall': 9.0}
    learning_rate =5e-4
    # ----------------------------
    # 6. 옵티마이저 정의
    # ----------------------------
    ...
    # ----------------------------
    n_epochs_adam = 20000
    n_epochs_lbgfs = 1#0000
    batch_size_adam = 2000
    ...
    # 11. 시각화 데이터 생성
    # ----------------------------
    time_values_animation = torch.linspace(0, 0.5, 20, device=device)
