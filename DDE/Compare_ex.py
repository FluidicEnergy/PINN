...
relative_Err=np.divide(np.abs(result_PINN_CFD-result_CFD),np.abs(result_CFD)+0.001)
Err=np.abs(result_PINN_CFD-result_CFD)
...
u=relative_Err[:,0].reshape(-1)
v=relative_Err[:,1].reshape(-1)
p=relative_Err[:,2].reshape(-1)

x, y, u, v, p= filter_close_points(x,y,u,v,p)

rbf_u = Rbf(x, y, u, function='linear')
rbf_v = Rbf(x, y, v, function='linear')
rbf_p = Rbf(x, y, p, function='linear')
...
for idx in range(3):
    ...
    axes[idx].scatter(samples[:, 0],
                ...
                cmap = 'jet',
                s = ssi)
    norm=colors.LogNorm(vmin=1e-4, vmax=5e1)
