...
class DeepONetPINN(nn.Module):
    ...
    이 모델은 공간 좌표 (x, y)를 입력으로 받는 Branch Network와,
    시간 t를 입력으로 받는 Trunk Network로 구성됩니다.
    두 네트워크의 출력은 concatenate한 후, 최종 fully-connected layer를 거쳐
    PDE의 해 (예: u, v, p)를 예측합니다.
    
    Args:
        branch_layers (list of int): Branch network의 layer 크기. 예를 들어, [2, 50, 50, 100]
            여기서 입력 2는 (x,y) 좌표입니다.
        trunk_layers (list of int): Trunk network의 layer 크기. 예를 들어, [1, 50, 50, 100]
            여기서 입력 1은 시간 t입니다.
        merge_hidden (int): Branch와 Trunk의 특징 결합 후 사용할 hidden layer 크기.
        output_dim (int): 최종 출력 차원 (예: 3은 (u, v, p)).
        activation (callable): 활성화 함수 (기본은 torch.tanh).
    """
    def __init__(self, branch_layers, trunk_layers, mu, rho, merge_hidden=50, output_dim=3, activation=torch.tanh):
        super(DeepONetPINN, self).__init__()
        self.activation = activation
        self.mu = mu
        self.rho = rho
        
        # Branch Network: 공간 입력 (x, y)
        branch_modules = []
        for i in range(len(branch_layers) - 1):
            branch_modules.append(nn.Linear(branch_layers[i], branch_layers[i+1]))
            # 마지막 레이어 이후에는 활성화를 사용하지 않아도 좋음.
            if i < len(branch_layers) - 2:
                branch_modules.append(nn.Tanh())
        self.branch_net = nn.Sequential(*branch_modules)

        # Trunk Network: 시간 입력 t
        trunk_modules = []
        ...
        # 결합 네트워크: Branch와 Trunk의 출력을 concatenate한 후 처리
        merge_input_dim = branch_layers[-1] + trunk_layers[-1]
        self.merge_fc = nn.Sequential(
            nn.Linear(merge_input_dim, merge_hidden),
            nn.Tanh(),
            nn.Linear(merge_hidden, output_dim)
        )

    def forward(self, spatial_input, time_input):
        """
        Args:
            spatial_input: Tensor of shape (N, 2) - (x, y)
            time_input: Tensor of shape (N, 1) - time t
        Returns:
            output: Tensor of shape (N, output_dim), e.g., (u, v, p)
        """
        branch_out = self.branch_net(spatial_input)   # (N, D_branch)
        trunk_out = self.trunk_net(time_input)          # (N, D_trunk)
        combined = torch.cat([branch_out, trunk_out], dim=1)  # (N, D_branch + D_trunk)
        output = self.merge_fc(combined)
        return output

    def net_uvp(self, x, y, t):
        """
        Convenience function that splits the output into u, v, p components.
        ...
        Returns:
            u, v, p: Each of shape (N, 1)
        """
        spatial_input = torch.cat([x, y], dim=1)  # (N,2)
        out = self.forward(spatial_input, t)       # (N, output_dim)
        u = out[:, 0:1]
        v = out[:, 1:2]
        p = out[:, 2:3]
        return u, v, p
    ...
    def net_f(self, x, y, t):
        ...
        f_u = u_t + (u * u_x + v * u_y) + p_x/self.rho - self.mu * (u_xx + u_yy)/self.rho
        f_v = v_t + (u * v_x + v * v_y) + p_y/self.rho - self.mu * (v_xx + v_yy)/self.rho
...
def plot_combined_loss(losses_mlp, losses_lbfgs, n_epochs_adam):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('comparison_loss_plot_MLP.jpg')
    #plt.show()
...
def train_pinn(
    ...
    def closure():
        ...
        # mini-batch sampling for interior
        idx_int = np.random.choice(len(x_interior), batch_size, replace=False)
        x_batch = x_interior[idx_int]
        y_batch = y_interior[idx_int]
        t_batch = torch.rand(batch_size, 1, device=device) * t_end

        # mini-batch sampling for boundary
        idx_bnd = np.random.choice(len(x_boundary), batch_size, replace=False)
        x_bnd = x_boundary[idx_bnd]
        y_bnd = y_boundary[idx_bnd]
        t_bnd = torch.rand(batch_size, 1, device=device) * t_end

        # PDE loss (interior)
        f_u, f_v, f_e = model.net_f(x_batch, y_batch, t_batch)
        loss_pde = torch.mean(f_u**2 + f_v**2 + f_e**2)

        # Boundary losses (using boundary mini-batch)
        u_pred, v_pred, p_pred = model.net_uvp(x_bnd, y_bnd, t_bnd)
        u_inlet, v_inlet, p_inlet = inlet_bc.apply(x_bnd, y_bnd, u_pred.clone(), v_pred.clone(), p_pred.clone())
        loss_inlet_bc = torch.mean((u_inlet - u_pred)**2) + torch.mean((v_inlet - v_pred)**2)

        u_top, v_top, p_top = top_bc.apply(x_bnd, y_bnd, u_pred, v_pred, p_pred)
        loss_top_bc = torch.mean((u_top - u_pred)**2) + torch.mean((v_top - v_pred)**2)

        u_bottom, v_bottom, p_bottom = bottom_bc.apply(x_bnd, y_bnd, u_pred, v_pred, p_pred)
        loss_bottom_bc = torch.mean((u_bottom - u_pred)**2) + torch.mean((v_bottom - v_pred)**2)

        _, _, p_outlet = outlet_p_bc.apply(x_bnd, y_bnd, u_pred, v_pred, p_pred)
        loss_outlet_p_bc = torch.mean((p_outlet - p_pred)**2)

        u_b_outlet_dy, v_b_outlet_dy, _ = outlet_du_dy_bc.apply(x_bnd, y_bnd, u_pred, v_pred, p_pred)
        loss_outlet_du_dy_bc = torch.mean((u_b_outlet_dy - u_pred)**2) + torch.mean((v_b_outlet_dy - v_pred)**2)

        u_wall, v_wall, p_wall = wall_bc.apply(x_bnd, y_bnd, u_pred.clone(), v_pred.clone(), p_pred.clone())
        ...
        # Initial condition loss using interior batch with t=0
        t_ic = torch.zeros(batch_size, 1, device=device)
        u_ic, v_ic, p_ic = model.net_uvp(x_batch, y_batch, t_ic)
        loss_ic = torch.mean((u_ic - torch.zeros_like(u_ic))**2 +
                             (v_ic - torch.zeros_like(v_ic))**2 +
                             (p_ic - torch.zeros_like(p_ic))**2)

        loss = loss_pde + bc_weights.get('inlet', 1.0)*loss_inlet_bc + \
               ...
               bc_weights.get('wall', 1.0)*loss_wall_bc + loss_ic
...
def create_unsteady_visualization_data(domain, model_mlp, num_x, num_y, lb, ub, time_values):
    ...
    for t in time_values:
        ...
        # 모델 예측
        u, v, p = model_mlp.net_uvp(x_flat, y_flat, t_flat)
    ...
    ani.save('unsteady_flow_MLP.gif', writer='pillow', fps=20)
...
if __name__ == "__main__":
    ...
    #)
    boundary_multiplyer = 2 
    x_interior, y_interior = generate_interior_points_cartesian(domain, num_x_int, num_y_int, lb, ub)
    x_boundary, y_boundary = generate_boundary_points_cartesian(domain, 2 * boundary_multiplyer* num_x_int, boundary_multiplyer*num_y_int, lb, ub, cylinder_center, cylinder_radius)
    time_values = torch.linspace(0, 0.5, 100, device=device)
    ...
    # 4. PINN 모델 초기화
    # ----------------------------
    branch_layers = [2, 50, 50, 100]
    trunk_layers = [1, 50, 50, 100]

    model = DeepONetPINN(branch_layers=branch_layers, trunk_layers=trunk_layers, mu=mu, rho=rho).to(device)
    
# ----------------------------
    # 5. 손실 가중치 정의
    # ----------------------------
    bc_weights = {'inlet': 1.0, 'top': 1.0, 'bottom': 1.0, 'outlet_p': 1.0, 'outlet_du_dy': 1.0, 'wall': 5.0}
    learning_rate =1e-4
    # ----------------------------
    # 6. 옵티마이저 정의
    # ----------------------------
    optimizer_mlp = optim.Adam(model.parameters(), lr=learning_rate)
    optimizer_lbfgs = optim.LBFGS(model.parameters(), lr=learning_rate, max_iter=50000, history_size=100, line_search_fn='strong_wolfe')
    ...
    X, Y, u_data, v_data, p_data = create_unsteady_visualization_data(domain, model, 100, 50, lb, ub, time_values_animation)
